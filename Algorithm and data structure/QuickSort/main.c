...
int main() {
    ...
    // Imprime a área após as inserções
    printf("Área após inserções:\n");
    ImprimeArea(&area);

    // Remove o menor item (primeiro)
    RetiraPrimeiro(&area, &item);
    printf("\nRemovido o menor (primeiro): %d\n", item.Chave);

    // Remove o maior item (último)
    RetiraUltimo(&area, &item);
    printf("Removido o maior (último): %d\n", item.Chave);

    // Imprime a área após as remoções
    printf("\nÁrea após remoções:\n");
    ImprimeArea(&area);
